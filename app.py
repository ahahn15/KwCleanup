...
family_terms = ['family', 'dad', 'daughter', 'father', 'children', 'boys',
                'girls', 'son', 'sisters', 'brothers', 'brother', 'sister', 'grandfather',
                'grandmother', 'mom', 'siblings']
...
def remove_asset_keywords():
    assets_to_process = request.json
    asf = json.dumps(assets_to_process)
    print(asf)
    asset_ids = json.loads(asf)
    ...
    for asset_id in asset_ids:
        ...
        print("Removed keywords from: ", asset_id, " response: ", response.content)
    return ""
...
def __call_asset_service(token, coord_id):
    ...
    params = '?assettype=image&family=creative&phrase=family%20AND%20animal%20AND%20NOT%20(' \
             'Digitally%20generated%20image%20OR%20illustration)&pagesize=160&begindate=1%2F1%2F2013&enddate=1%2F1%2F2015&deliverysizes' \
